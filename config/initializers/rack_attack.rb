...
class Rack::Attack
  ...
  throttle("throttle logins by email", limit: 5, period: 20.seconds) do |req|
    req.params.dig("user", "email").to_s.scrub.downcase if req.path == "/users/sign_in" && req.post?
  end
  ...
  throttle("throttle password resets by email", limit: 5, period: 20.seconds) do |req|
    req.params.dig("user", "email").to_s.scrub.downcase if req.path == "/users/password" && req.post?
  end
