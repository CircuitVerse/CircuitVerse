class ProjectsQuery
  ...
  def full_text_search_name_description(query)
    relation.includes(:author).search(query).select("id,author_id,image_preview,name,description,view")
  end

  def simple_search_name_description(query)
    relation.includes(:author).where("name LIKE ? OR description LIKE ?","%#{query}%","%#{query}%").select("id,author_id,image_preview,name,description,view")
  end
