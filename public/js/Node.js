...
function replace(node, index) {
    ...
    node.delete();
    const newNode = scope.allNodes[index];
    newNode.parent = parent;
    parent.nodeList.push(newNode);
    newNode.updateRotation();
    return newNode;
}

function extractBits(num, start, end) {
    return (num << (32 - end)) >>> (32 - (end - start + 1)); // eslint-disable-line no-bitwise
}
...
Node.prototype.setLabel = (label) => {
    this.label = label; // || "";
};


Node.prototype.converToIntermediate = () => {
    this.type = 2;
    this.x = this.absX();
    this.y = this.absY();
...
Node.prototype.absX = () => (this.x + this.parent.x);

Node.prototype.absY = () => (this.y + this.parent.y);

Node.prototype.updateScope = (scope) => {
    this.scope = scope;
    if (this.type === 2) this.parent = scope.root;
};

Node.prototype.isResolvable = () => this.value !== undefined;

Node.prototype.reset = () => {
    this.value = undefined;
    this.highlighted = false;
};

Node.prototype.connect = (n) => {
    if (n === this) return;
...
Node.prototype.connectWireLess = (n) => {
    if (n === this) return;
    if (n.connections.contains(this)) return;
    this.connections.push(n);
    n.connections.push(this);
...
Node.prototype.disconnectWireLess = (n) => {
    ...
    if (!this.wasClicked && this.clicked) {
        ...
        if (this.type === 2) {
            ...
            if (this.connections.length === 1 && this.connections[0].absX() === this.absX() && this.connections[0].absY() === this.absY()) {
                ...
                [simulationArea.lastSelected] = this.connections;
                this.delete();
                return;
            }
        }
...
Node.prototype.isClicked = () => (this.absX() === simulationArea.mouseX && this.absY() === simulationArea.mouseY);

Node.prototype.isHover = () => (this.absX() === simulationArea.mouseX && this.absY() === simulationArea.mouseY);

Node.prototype.nodeConnect = () => {
    const x = this.absX();
    const y = this.absY();
    let n;
