...
$(document).ready(function () {
    ...
    $("#clockProperty").append(
        "<label class=''> <input type='button' class='objectPropertyAttribute' name='toggleFullScreen' value='Fullscreen' style='font-size: 20px'> </input> </label></br>"
    );
    $("#clockProperty").append(
        "Time: <input class='objectPropertyAttribute' min='50' type='number' style='width:48px' step='10' name='changeClockTime'  value='" +
            simulationArea.timePeriod +
            "'><br>"
    );
    $("#clockProperty").append(
        "Clock: <label class='switch'> <input type='checkbox' " +
            ["", "checked"][simulationArea.clockEnabled + 0] +
            " class='objectPropertyAttributeChecked' name='changeClockEnable' > <span class='slider'></span> </label><br>"
    );
...
function toggleFullScreen(value) {
    if (!getfullscreenelement()) GoInFullscreen(document.documentElement);
...
function exitHandler() {
    setTimeout(function () {
        for (id in scopeList) scopeList[id].centerFocus(true);
        gridUpdate = true;
        scheduleUpdate();
    }, 100);
}
