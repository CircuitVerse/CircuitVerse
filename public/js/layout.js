...
function renderLayout(scope = globalScope) {
    ...
    rect2(
        ...
        0,
        0,
        temp_buffer.layout.width,
        temp_buffer.layout.height,
        xx,
        yy,
        "RIGHT"
    );
    ...
    if (temp_buffer.layout.titleEnabled) {
        fillText(
            ctx,
            scope.name,
            temp_buffer.layout.title_x + xx,
            yy + temp_buffer.layout.title_y,
            11
        );
    }
    ...
    for (var i = 0; i < temp_buffer.Input.length; i++) {
        ...
        var info = determine_label(
            temp_buffer.Input[i].x,
            temp_buffer.Input[i].y,
            scope
        );
        ctx.textAlign = info[0];
        fillText(
            ctx,
            temp_buffer.Input[i].label,
            temp_buffer.Input[i].x + info[1] + xx,
            yy + temp_buffer.Input[i].y + info[2],
            12
        );
    }
    for (var i = 0; i < temp_buffer.Output.length; i++) {
        ...
        var info = determine_label(
            temp_buffer.Output[i].x,
            temp_buffer.Output[i].y,
            scope
        );
        ctx.textAlign = info[0];
        fillText(
            ctx,
            temp_buffer.Output[i].label,
            temp_buffer.Output[i].x + info[1] + xx,
            yy + temp_buffer.Output[i].y + info[2],
            12
        );
    }
    ctx.fill();
...
function layoutResetNodes() {
    ...
    temp_buffer.layout.height =
        Math.max(temp_buffer.Input.length, temp_buffer.Output.length) * 20 + 20;
    for (var i = 0; i < temp_buffer.Input.length; i++) {
...
layoutNode.prototype.update = function () {
    ...
    if (!this.wasClicked && this.clicked) {
        ...
        if (
            temp_buffer.isAllowed(
                simulationArea.mouseX - this.xx,
                simulationArea.mouseY - this.yy
            ) &&
            !temp_buffer.isNodeAt(
                simulationArea.mouseX - this.xx,
                simulationArea.mouseY - this.yy
            )
        ) {
...
function saveLayout() {
    if (layoutMode) {
        for (var i = 0; i < temp_buffer.Input.length; i++) {
            temp_buffer.Input[i].parent.layoutProperties.x =
                temp_buffer.Input[i].x;
            temp_buffer.Input[i].parent.layoutProperties.y =
                temp_buffer.Input[i].y;
        }
        for (var i = 0; i < temp_buffer.Output.length; i++) {
            temp_buffer.Output[i].parent.layoutProperties.x =
                temp_buffer.Output[i].x;
            temp_buffer.Output[i].parent.layoutProperties.y =
                temp_buffer.Output[i].y;
        }
