...
var plotArea = {
    ...
    plot: function () {
        ...
        document.getElementById("plot").style.height = Math.min(
            this.c.height,
            400
        );
        ...
        if (this.scroll) {
            this.ox = Math.max(
                0,
                (time / this.unit) * this.pixel - this.c.width + 70
            );
            ...
            for (; j < arr.length; j++) {
                ...
                if (start <= time) {
                    if (globalScope.Flag[i].bitWidth == 1) {
                        ...
                        ctx.lineTo(
                            80 + start * unit - plotArea.ox,
                            2 * (30 + i * 15 - yOff) - plotArea.oy
                        );
                        ctx.lineTo(
                            80 + end * unit - plotArea.ox,
                            2 * (30 + i * 15 - yOff) - plotArea.oy
                        );
                    } else {
                        ctx.beginPath();
                        ...
                        ctx.lineTo(
                            80 + end * unit - 5 - plotArea.ox,
                            2 * (25 + i * 15) - plotArea.oy
                        );
                        ctx.lineTo(
                            80 + start * unit + 5 - plotArea.ox,
                            2 * (25 + i * 15) - plotArea.oy
                        );
                        ctx.lineTo(
                            80 + start * unit - plotArea.ox,
                        ...
                        mid =
                            80 +
                            (((end + start) /
                                Math.round(plotArea.unit * plotArea.scale)) *
                                this.pixel) /
                                2;
                        ctx.font = "12px Georgia";
                        ctx.fillStyle = "white";
                        if (
                            start * unit + 10 - plotArea.ox <= 0 &&
                            end * unit + 10 - plotArea.ox >= 0
                        ) {
                            mid =
                                80 +
                                ((end - 3000) /
                                    Math.round(
                                        plotArea.unit * plotArea.scale
                                    )) *
                                    this.pixel;
                        }
        ...
        for (
            var i = 1;
            i * Math.round(plotArea.unit * plotArea.scale) <=
            time + Math.round(plotArea.unit * plotArea.scale);
            i++
        ) {
            ctx.fillText(
                (Math.round(plotArea.unit * plotArea.scale) * i) / 1000 + "s",
                48 + this.pixel * i - plotArea.ox,
                20
            );
        }
        ...
        var specificTime =
            ((plotArea.specificTimeX + plotArea.ox - 80) *
                Math.round(plotArea.unit * plotArea.scale)) /
            this.pixel;
        ...
        if (1.115 * plotArea.specificTimeX >= 80) {
            ...
            ctx.fillStyle = "black";
            ctx.fillText(
                Math.round(specificTime) + "ms",
                plotArea.specificTimeX - 25,
                20
            );
        }
        // borders
