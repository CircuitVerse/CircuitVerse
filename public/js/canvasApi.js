...
/*
 * function to draw Dots on screen
...
function dots(points = true, transparentBackground = false, force = false) {
    if (points) {
        // do something here with dots, not removed cause removing it would change how other functions call this function
    }
...
function moveTo(ctx, x1, y1, xx, yy, dir, bypass = false) {
    ...
    xx *= globalScope.scale;
    yy *= globalScope.scale;
    if (bypass) ctx.moveTo(xx + globalScope.ox + newX, yy + globalScope.oy + newY);
    else ctx.moveTo(Math.round(xx + globalScope.ox + newX - correction) + correction, Math.round(yy + globalScope.oy + newY - correction) + correction);
}
