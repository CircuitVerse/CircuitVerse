...
function changeInputSize(size) {
    ...
    var obj = new window[this.objectType](
        this.x,
        this.y,
        this.scope,
        this.direction,
...
SixteenSegDisplay.prototype.customDrawSegment = function (
    x1,
    y1,
    x2,
    y2,
...
SixteenSegDisplay.prototype.customDrawSegmentSlant = function (
    x1,
    y1,
    x2,
    y2,
...
ForceGate.prototype.resolve = function () {
    if (this.inp2.value != undefined) this.output1.value = this.inp2.value;
    else this.output1.value = this.inp1.value;
    simulationArea.simulationQueue.add(this.output1);
};
...
Text.prototype.keyDown = function (key) {
    if (key.length == 1) {
        if (this.label == "Enter Text Here") this.setLabel(key);
        else this.setLabel(this.label + key);
    } else if (key == "Backspace") {
        if (this.label == "Enter Text Here") this.setLabel("");
        else this.setLabel(this.label.slice(0, -1));
    }
};
...
Text.prototype.draw = function () {
    ...
    if (
        ...
        rect2(
            ctx,
            -this.leftDimensionX,
            -this.upDimensionY - magicDimenstion,
            this.leftDimensionX + this.rightDimensionX,
            this.upDimensionY + this.downDimensionY + magicDimenstion,
            this.x,
            this.y,
            "RIGHT"
        );
        ...
        for (var i = 0; i < this.splitCount; i++) {
            ...
            if (this.bitWidthSplit[i] > 1)
                var label =
                    ...
                    bitCount +
                    "]";
            else var label = this.inp1.verilogLabel + "[" + bitCount + "]";
...
function Input(
    ...
    if (layoutProperties) this.layoutProperties = layoutProperties;
    else {
...
Output.prototype.customDraw = function () {
    ...
    ctx.fillStyle = "green";
    ctx.textAlign = "center";
    if (this.state === undefined) var bin = "x".repeat(this.bitWidth);
    else var bin = dec2bin(this.state, this.bitWidth);
...
BitSelector.prototype.customDraw = function () {
    ...
    ctx.fillStyle = "green";
    ctx.textAlign = "center";
    if (this.bitSelectorInp.value === undefined) var bit = "x";
    else var bit = this.bitSelectorInp.value;
...
Demultiplexer.prototype.resolve = function () {
    for (var i = 0; i < this.output1.length; i++) this.output1[i].value = 0;
...
Flag.prototype.setPlotValue = function () {
    ...
    if (this.plotValues[this.plotValues.length - 1][1] == this.inp1.value)
        return;
    else this.plotValues.push([time, this.inp1.value]);
};
...
Flag.prototype.customDraw = function () {
    ...
    else fillText(ctx, "x", xx + 35 - this.xSize, yy + 8, 25);
    ctx.fill();
};
...
Tunnel.prototype.customDraw = function () {
    ...
    else fillText(ctx, "x", xx - 23 + xRotate, yy + 8 + yRotate, 25);
    ctx.fill();
};
...
ALU.prototype.resolve = function () {
    if (this.controlSignalInput.value == 0) {
        ...
        this.message = "A-B";
        this.output.value =
            ((this.inp1.value - this.inp2.value) << (32 - this.bitWidth)) >>>
            (32 - this.bitWidth);
        simulationArea.simulationQueue.add(this.output);
        this.carryOut.value = 0;
        simulationArea.simulationQueue.add(this.carryOut);
        ...
        this.message = "A<B";
        if (this.inp1.value < this.inp2.value) this.output.value = 1;
