...
$("#customShortcut").click(() => {
	...
	$("#customShortcutDialog").dialog({
		...
		create: function () {
			...
			$(
				"<label class='disable--cb' ><input id='' onchange='toggle_hotkey()' type='checkbox'/> Enable </label>"
			).prependTo(pane);
			if (
				localStorage.get("custom_hotkey") &&
				localStorage.get("custom_hotkey").enable === true
			)
				$(".disable--cb > input[type='checkbox']").prop(
					"checked",
					true
				);
		},
