...
function loadSimulatorDisabledPage() {
    if(window.history) {
        ...
        $('#continue-anyway').click(function() {
            Cookies.set('disableMobileCheck', true);
            window.history.back();
        });
    }
    else {
        $('#go-back-button').html("Browser not supported");
        $('#go-back-button').prop( "disabled", true );
        $('#continue-anyway').html("Go Back function is not implemented in your browser");
    }
}