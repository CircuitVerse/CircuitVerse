    ...
    for (var i = 0; i < this.testData.inputs.length; i++) {
        this.outputs.push(
            new Node(
                ...
                1,
                this,
                (bitWidth = this.testData.inputs[i].bitWidth),
                (label = this.testData.inputs[i].label)
            )
        );
    }
        ...
        for (var i = 0; i < this.testBenchInput.testData.outputs.length; i++) {
            this.inputs.push(
                new Node(
                    ...
                    NODE_INPUT,
                    this,
                    (bitWidth = this.testBenchInput.testData.outputs[i]
                        .bitWidth),
                    (label = this.testBenchInput.testData.outputs[i].label)
                )
            );
        }
    }
        ...
        if (this.testBenchInput.running && this.testBenchInput.iteration) {
            ...
            for (
                ...
                if (this.inputs[i].value != undefined) {
                    ...
                    fillText(
                        ctx,
                        dec2bin(this.inputs[i].value, this.inputs[i].bitWidth),
                        xx + this.rightDimensionX / 2,
                        30 + i * 20 + yy + 4,
                        10
                    );
