verilog = {
    ...
    exportVerilogScope: function (scope = globalScope) {
        ...
        var output = this.exportVerilogScope_r(
            scope.id,
            completed,
            dependencyList
        );
    ...
    exportVerilogScope_r: function (id, completed = {}, dependencyList = {}) {
        ...
        for (bitWidth in scope.verilogWireList) {
            if (bitWidth == 1)
                output +=
                    "wire " + scope.verilogWireList[bitWidth].join(",") + ";\n";
            else
                output +=
                    "wire [" +
                    (bitWidth - 1) +
                    ":0] " +
                    scope.verilogWireList[bitWidth].join(",") +
                    ";\n";
        }
        output += res;
    ...
    getReady: function (scope = globalScope) {
        ...
        for (var i = 0; i < scope.ConstantVal.length; i++) {
            if (scope.ConstantVal[i].label == "")
                ...
                scope.ConstantVal[i].label = this.fixName(
    ...
    generateHeader: function (scope = globalScope) {
        return (
            "module " +
            this.fixName(scope.name) +
            " (" +
            scope.Input.map(function (x) {
                return x.label;
            }).join(",") +
            "," +
            scope.Output.map(function (x) {
                return x.label;
            }).join(",") +
            ");\n"
        );
