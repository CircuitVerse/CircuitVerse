 ...
 */
 export function resetup() {
    ...
    // redraw grid
    dots();
    document.getElementById('backgroundArea').style.height = ((height / DPR) + 100 + "px");
    document.getElementById('backgroundArea').style.width = ((width / DPR) + 100 + "px");
...
function setupElementLists() {
    ...
    function createIcon(element) {
        ...
        return `<div class="icon logixModules" id="${element.name}" title="${element.label}">
            <img src= "${imgToDisp}" alt="${element.name}" >
        </div>`;
    }
