...
RSpec.describe Api::V1::GroupsController, "#groups_owned", type: :request do
  describe "list all groups owned" do
    ...
    context "when authenticated as owner and including assignments" do
      before do
        # create 3 groups with assignments and group_members for each
        FactoryBot.create_list(:group, 3, owner: owner).each do |g|
      ...
      it "returns all groups including assignments signed in user owners" do
    ...
    context "when authenticated as owner and including group_members" do
      before do
      ...
      it "returns all groups including group_members signed in user owners" do
        expect(response).to have_http_status(:ok)
