...
import {
    setProjectName, getProjectName, generateImage, generateSaveData,
} from './data/save';
...
export function showProperties(obj) {
    ...
    if (layoutModeGet()) {
        ...
        if (obj.subcircuitMutableProperties && obj.canShowInSubcircuit) {
            for (const attr in obj.subcircuitMutableProperties) {
                const prop = obj.subcircuitMutableProperties[attr];
                if (obj.subcircuitMutableProperties[attr].type === 'number') {
                    const s = `<p>${prop.name}<input class='objectPropertyAttribute' type='number'  name='${prop.func}' min='${prop.min || 0}' max='${prop.max || 200}' value=${obj[attr]}></p>`;
                    $('#moduleProperty-inner').append(s);
                } else if (obj.subcircuitMutableProperties[attr].type === 'text') {
                    const s = `<p>${prop.name}<input class='objectPropertyAttribute' type='text'  name='${prop.func}' maxlength='${prop.maxlength || 200}' value=${obj[attr]}></p>`;
                    $('#moduleProperty-inner').append(s);
                } else if (obj.subcircuitMutableProperties[attr].type === 'checkbox') {
                    const s = `<p>${prop.name}<label class='switch'> <input type='checkbox' ${['', 'checked'][obj.subcircuitMetadata.showLabelInSubcircuit + 0]} class='objectPropertyAttributeChecked' name='${prop.func}'> <span class='slider'></span> </label></p>`;
                    $('#moduleProperty-inner').append(s);
                }
            }
            let subCircuitLabelDirection;
            if (!obj.labelDirectionFixed) {
                if (!obj.subcircuitMetadata.labelDirection) subCircuitLabelDirection = obj.labelDirection;
                const s = `<select class='objectPropertyAttribute' name='newLabelDirection'> <option value='RIGHT' ${['', 'selected'][(subCircuitLabelDirection === 'RIGHT')]} >RIGHT</option><option value='DOWN' ${['', 'selected'][(subCircuitLabelDirection === 'DOWN')]} >DOWN</option> <option value='LEFT' <option value='RIGHT'${['', 'selected'][(subCircuitLabelDirection === 'LEFT')]} >LEFT</option><option value='UP' <option value='RIGHT'${['', 'selected'][(subCircuitLabelDirection === 'UP')]} >UP</option> </select>`;
                s.val(subCircuitLabelDirection);
        ...
        if (!obj.disableLabel) $('#moduleProperty-inner').append(`<p><span>Label:</span> <input class='objectPropertyAttribute' type='text'  name='setLabel' autocomplete='off'  value='${escapeHtml(obj.label)}'></p>`);
...
function setupPanelListeners(panelSelector) {
    ...
    $(panelSelector).on('mousedown', () => {
        ...
        $('#quick-btn-panel').css('z-index', '99');
...
export function setupPanels() {
    ...
    $('#quick-btn-panel').on('mousedown', () => {
        $('.draggable-panel').css('z-index', '99');
        $('#quick-btn-panel').css('z-index', '100');
    });
...
export function fillSubcircuitElements() {
    ...
    $('.subcircuitModule').mousedown(function () {
        const { elementName } = this.dataset;
