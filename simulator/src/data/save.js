 ...
 * @category data
 */
function afterSave(data, projectName, imageData) {
    if (!userSignedIn) {
...
function afterDialogBoxSave(name) {
    ...
    afterSave(data, getProjectName(), generateImageForOnline());
}

function saveOnlineDialogBox(name) {
    $("#saveProject").empty();
    $("#saveProject").append(
        `<label style="margin: 0;">Enter Project name: </label> <input  id='newProjectName' required type='text'>`
    );
    $("#saveProject").dialog({
        width: "auto",
        buttons: [
...
export function saveOfflineData(name) {
    data = {};
