...
import { scheduleUpdate, updateSimulationSet, updateSubcircuitSet, forceResetNodesSet } from './engine';
...
export function paste (copyData) {
  ...
  for (let i = 0; i < data.scopes.length; i++) {
    if (scopeList[data.scopes[i].id] === undefined) {
      ...
      const scope = newCircuit(data.scopes[i].name, data.scopes[i].id, isVerilogCircuit, isMainCircuit);
