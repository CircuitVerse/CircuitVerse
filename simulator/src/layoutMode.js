...
import { hideProperties, fillSubcircuitElements, prevPropertyObjGet, showProperties } from './ux';
...
export function renderLayout (scope = globalScope) {
  ...
  for (let i = 0; i < tempBuffer.subElements.length; i++) {
    ...
    // element nodes
    for (let j = 0; j < tempBuffer.subElements[i].nodeList.length; j++) { tempBuffer.subElements[i].nodeList[j].update(); }
  }
